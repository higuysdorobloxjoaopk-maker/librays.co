...
-- // Here we set up our global variables and configuration tables
getgenv().kms = false
local Ihatethisui = {}
local UIName = 'luy' -- We changed the library name here!
local Amount = 0
local ConfigF
...
-- // This is our collection of beautiful themes. Feel free to add more later!
local Themes = {
    ['Default'] = {
...
-- // Utility Functions - The behind the scenes magic that makes our UI pop
do
    -- // This helps us smoothly animate elements
    function Utility:Tween(Instance, Properties, Duration, ...)
        local TweenInfo = TweenInfo.new(Duration, ...)
        TweenService:Create(Instance, TweenInfo, Properties):Play()
    end

    -- // Removes the UI completely from the game
    ...
    -- // Little helper to make colors slightly darker
    ...
    -- // Little helper to make colors slightly lighter
    ...
    -- // Extracts RGB components out of a Color3 value
    function Utility:SplitColor(Color)
        local R, G, B = math.floor(Color.R * 255), math.floor(Color.G * 255), math.floor(Color.B * 255)
        return {R, G, B}
    end

    -- // Joins an RGB array back into a Color3 value
    ...
    -- // Makes our UI draggable (Now updated for Mobile Touch support!)
    function Utility:EnableDragging(Frame)
        ...
        Frame.InputBegan:Connect(function(Input)
            -- We listen to both Mouse Clicks and Touch events for mobile gamers!
        ...
        Frame.InputChanged:Connect(function(Input)
            -- Track movement across the screen, whether it's a cursor or a finger
        ...
        UserInputService.InputChanged:Connect(function(Input)
            -- Actually update the frame's position while holding
    ...
    -- // A quick wrapper to instance objects with properties nicely
    ...
    -- // This builds those nice little pop-ups in the corner of your screen
    function Library:CreateNotification(Title, Text, Duration)
        local Theme = {}
        local File = readfile('luyCurrentTheme.json')
        ...
        task.spawn(function()
            ...
            if not CoreGui:FindFirstChild('luy | Notifications') then
                Utility:Create('ScreenGui', {
                    Name = 'luy | Notifications',
                    Parent = CoreGui
                })
            else
                Utility:Create('Frame', {
                    Parent = CoreGui:FindFirstChild('luy | Notifications'),
                ...
                Amount = Amount + 1
                local Holder = CoreGui:FindFirstChild('luy | Notifications')['Notification'..tostring(Amount)]
    ...
    -- // Prompts for user confirmations (Text input, One Button, or Two Buttons)
...
-- // Startup cleanup: ensure we don't overlap loops if the UI is run multiple times
do 
    Utility:DestroyUI()
    BreakAllLoops = false
    task.spawn(function()
...
-- // Main Library Functions - This is where the magic happens for saving configs, tweaking visuals, etc.
...
function Library:SetTransparency(Transparency, NotificationBool)
    local UI = CoreGui:WaitForChild('luy')
    local Blacklist = {
        'UIPadding',
        'UICorner',
        'UIStroke',
...
-- // Loading configs back up into the UI
...
-- // Here's where we start crafting the actual UI window!
function Library:CreateWindow(HubName, GameName, IntroText, IntroIcon, ImprovePerformance, ConfigFolder, Theme)
    ...
    if isfile('luyCurrentTheme.json') then
        delfile('luyCurrentTheme.json')
    end

    ConfigF = ConfigFolder

    writefile('luyCurrentTheme.json', HttpService:JSONEncode(NewTable))

    Config['Theme_4z3s4QrUhfqt703FmiAe'] = HttpService:JSONEncode(NewTable)

    -- // Building the physical container for our UI
    ...
    -- // Make the main window draggable across the screen
    Utility:EnableDragging(Container.Main)

    -- // Little intro animation sequence
    ...
    -- // Assembling all the structural pieces of the window
    Utility:Create('Frame', {
        Name = 'PromptHolder',
        Parent = Main,
        AnchorPoint = Vector2.new(0.5, 0.5),
    ...
    -- // Main visual update loop for dynamic theming
    ...
    -- // Tabs API to segregate content cleanly
    local Tabs = {}
    ...
    function Tabs:CreateTab(TabName, DefaultVisibility, Icon, RectOffset, RectSize)
        ...
        -- // Sections sit inside Tabs to group your elements up nicely
        local Sections = {}
        ...
        function Sections:CreateSection(Name)
            ...
            -- // Finally, our UI Elements! Checkboxes, sliders, buttons, etc.
            local Elements = {}
